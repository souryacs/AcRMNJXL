...
def parse_options():  
  ...
  #parser.add_option("-m", "--method", \
			  ...
			  #2 - mode based accumulated coalescence rank of the couplets (AcRMNJ) \
			  #3 - product of mean coalescence rank (LCA rank) & mean accumulated coalescence rank between couplets, with mode statistics (MRAcRMNJ) \
			  #4 - LCA rank with mode statistics (RMNJ) (mode based adaptation of STAR method) \
			  #5 - mode based accumulated coalescence rank with extra lineage information (AcRMNJXL) \
			  #6 - product of mean coalescence rank (LCA rank) & mean accumulated coalescence rank between couplets (MRAcRNJ)")
  ...
  parser.add_option("-m", "--method", \
			  ...
			  help="1 - average accumulated coalescence rank of the couplets (AcRNJ) (Default Method) \
			  2 - mode based accumulated coalescence rank of the couplets (AcRMNJ) \
			  3 - mode based accumulated coalescence rank with extra lineage information (AcRMNJXL)")
...
def main():  
  ...
  if (OUTPUT_FILENAME == ""):
    ...
    if (METHOD_USED == AcRNJ): 
      ...
      dir_of_curr_exec = dir_of_inp_file + 'AcRMNJ'
    elif (METHOD_USED == AcRMNJXL):
      dir_of_curr_exec = dir_of_inp_file + 'AcRMNJXL'
  ...
  Form_Species_Tree_NJ_Cluster(Star_Tree_Initial, COMPLETE_INPUT_TAXA_LIST, METHOD_USED, NJ_RULE_USED, Output_Text_File)
